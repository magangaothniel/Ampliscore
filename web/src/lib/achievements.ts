 ...
 * Badges.
 *
 * Evaluation runs on the client and inserts into `achievements`, which is
 * protected by RLS so a user can only ever write rows for themselves and can
 * never update or delete one. That still means a determined student could
 * insert a badge from the browser console. Acceptable while badges are purely
 * cosmetic. If they ever appear on a leaderboard or unlock anything, this must
